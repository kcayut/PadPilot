...
- **自動情境接管**：
  - **外出 / Headless 模式**：Mac mini 未連接任何實體螢幕時，只要偵測到指定的 iPad 透過 USB-C 連接，立即自動建立 Sidecar 連線並將 iPad 設為主要顯示器（Main Display）。
...
- **手動操作絕對優先 (User Override)**：
  ...
  - 導入 **Topology Generation（拓撲世代碼）** 機制：手動決策會綁定當前硬體世代，只要硬體連接未發生實質改變，自動化絕不推翻使用者的決定；只有在實體拔除螢幕後才自動失效並重新評估。
- **單飛行狀態轉換鎖 (Single-Flight Transition Lock)**：
  - 防止 USB 插入、螢幕喚醒、定時輪詢併發觸發多次 Sidecar 連線，保障連線不發生 Race Condition。
- **連線防抖與冷卻保護**：
  - **Debounce 防抖**：實體螢幕瞬斷或切換輸入源時，啟動 4 秒防抖計時，未滿前不動作；螢幕於 4 秒內恢復立即取消切換。
...
- **事件主動刷新 + 30 秒 Watchdog**：
  - 狀態變更時即時呼叫 `open -g "swiftbar://refreshplugin?plugin=padpilot.30s.py"` 更新 Menu Bar。
...
| 模式 | 行為說明 |
| :--- | :--- |
| **Automatic**<br>*(預設日常)* | 有實體螢幕時不自動干預；無實體螢幕且配對 USB iPad 連接時，自動 Sidecar 並將 iPad 設為主螢幕。 |
...
專案具備完整且嚴格的單元測試套件（共 18 項測試，100% 通過），涵蓋驗收標準中的 10 大完整情境以及 LaunchAgent 登入啟動管理：
...
7. **Scenario 7 (喚醒)**：Mac 喚醒後觸發評估，若狀態已滿足則不無故重連。

...
診断の FileVault と macOS 自動ログインは橙色の情報表示とし、インストールの合格・不合格には使用しません。
...
**症状：** メニューに警告と 30 秒のクールダウンが表示されます。

接続試行は最大 3 回、間隔は 3 秒です。失敗後は 30 秒待機し、macOS や BetterDisplay への連続接続要求を抑えます。
...
- `WARN: Tkinter`：設定画面だけが無効になります。メニュー、daemon、CLI は利用できます。導入時に選んだ Python に対応する Tk を追加し、設定画面を開き直してください。

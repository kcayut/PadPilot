...
def build_dmg(app, destination):
    ...
    with tempfile.TemporaryDirectory(prefix='padpilot-dmg-') as directory:
        ...
        dmgbuild.build_dmg(str(destination), 'PadPilot', settings={
            'format': 'UDZO',
            'files': [(str(app), 'PadPilot.app'), str(link), str(ROOT / 'assets/dmg/Start Here.html')],
            ...
            'icon_locations': {'PadPilot.app': (320, 326), 'Applications': (610, 326),
    ...
    with tempfile.TemporaryDirectory(prefix='padpilot-dmg-check-') as directory:
        ...
        try:
            ...
            subprocess.run(['codesign', '--verify', '--deep', '--strict', str(mount / 'PadPilot.app')], check=True)
        finally:

...
詳しくは[トラブルシューティング](docs/TROUBLESHOOTING.ja.md)へ。App 内のヘルプは、選択言語・対象バージョンの GitHub 文書を開きます。非公開リポジトリでは閲覧権限のあるアカウントが必要です。
...
**リリース版を削除するには、メニューの「終了」を選び、Applications の PadPilot.app をゴミ箱へ移します。アンインストールボタンは不要です。** ログインサービスはアプリ内にあり、macOS が管理します。ゴミ箱内のアプリはバックグラウンド Python を起動しません。設定・ペアリング・ログは保持され、ログイン項目の名前が消えるまで時間がかかる場合があります。
